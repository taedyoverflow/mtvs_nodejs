 ...
 * 브라우저 상에서 동작하는 js 는 script 태그를 이용하여 여러 개의 script 파일을 하나로 합쳐 사용한다.
 * 이 때 js 파일들은 동일한 유효범위(scope)를 가지게 되어 전역변수 중복과 같은 문제가 발생할 수 있었다.
 * 
 ...
 * 클라이언트 사이드에서 발생하던 전역변수 중복 문제가 발생하지 않게 된다.
 * 
 * 모듈은 module.exports 또는 exports 객체를 통해 정의하고 외부로 공개하며,
 * 공개된 모듈은 require 함수를 사용하여 임포트 할 수 있다.
 * (export, export default, import 방식은 ES6이후에 추가된 문법이기 때문에
